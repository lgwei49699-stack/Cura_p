...
Item
{
    ...
    Item
    {
        ...
        // 云端导入按钮（仅 Explorer 3 且在自定义模式下显示）
        Cura.SecondaryButton
        {
            id: cloudImportButton
            anchors.top: parent.top
            anchors.right: uploadConfigButton.left
            anchors.margins: parent.padding
            leftPadding: UM.Theme.getSize("default_margin").width
            rightPadding: UM.Theme.getSize("default_margin").width
            text: "云端导入"
            visible: currentModeIndex == PrintSetupSelectorContents.Mode.Custom && UM.ConfigUploadHandler.isExplorer3Machine
            
            onClicked: {
                cloudImportDialog.visible = true
            }
        }
        
        // 上传配置按钮（仅 Explorer 3 且在自定义模式下显示）
        Cura.SecondaryButton
        {
            id: uploadConfigButton
            anchors.top: parent.top
            ...
            onClicked: {
                uploadConfigDialog.visible = true
            }
        }
    ...
    // 云端导入对话框
    Dialogs.CloudImportDialog
    {
        id: cloudImportDialog
    }
    
    // 上传配置对话框
    Dialogs.UploadConfigDialog
    {
